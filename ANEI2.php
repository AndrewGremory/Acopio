	...
	<script type="text/javascript">
		...
		{ 
			...
			{ 
			  ...
			  document.getElementById('precio_final').value=moja1;
			  document.getElementById('precio_final1').value=moja;
			}
			...
			var f = document.getElementById('precio_final1').value;
			...
			document.getElementById('subtotal').value= cesar;
			document.getElementById('subtotal1').value= h;
			...
			{ 				
				...
				document.getElementById('dian').value= j1;
				document.getElementById('aporte').value= j1;
				document.getElementById('dian1').value= j;
				document.getElementById('aporte1').value=j; 
			}
			...
			document.getElementById('asociados').value= k1;
			document.getElementById('asociados1').value= k;
			...
			document.getElementById('neto_pagar').value= l1;
			document.getElementById('neto_pagar1').value= l;
		}		
		function facr() 
		...
		{   			        		
			...
			var cesar= new Intl.NumberFormat('de-DE').format(tot);
			
			document.getElementById('costo_total_resultado').value =cesar;
			document.getElementById('costo_total_resultado1').value =tot;
		}
		...
		{   		
			...
			document.getElementById('total_ahorro').value = kk1;
			document.getElementById('total_ahorro1').value = kk;
             
			var tp = parseInt(np) - kk - parseInt(ap);

			var tp1= new Intl.NumberFormat('de-DE').format(tp);
			document.getElementById('neto_pagar2').value = tp1;
			document.getElementById('neto_pagar21').value = tp;
		}
		...
		function analisis_fisico()
		{
          swal({
		  position: "top-end",
		  title: "¡Alerta!",
          text: "No se ha Realizado el Analisis Fisico",
          type: "error",
          });
        }
		...
		function dest()
		{
			var destare = document.getElementById('destare').value;
			var kilos = document.getElementById('kilos').value;
			var t_destare = parseInt(kilos) - parseInt(destare);
			document.getElementById('t_kilos').value = t_destare;
        }

		function fijar()
		{
			if (document.getElementById('producto').value == document.getElementById('producto1').value ) 
			{ 
				
				document.getElementById("precio_fijado").value = document.getElementById("precio_fijado1").value;
				document.getElementById("cantidad_Fijada").value = document.getElementById("cantidad_Fijada1").value;
				document.getElementById("kilos_vendidos").value = document.getElementById("kilos_vendidos1").value;
				document.getElementById("cupo_disponible_fijacion").value = document.getElementById("cupo_disponible_fijacion1").value;				
			}
			if (document.getElementById('producto').value != document.getElementById('producto1').value ) 
			{				
				document.getElementById("precio_fijado").value = 0;
				document.getElementById("cantidad_Fijada").value = 0;
				document.getElementById("kilos_vendidos").value = 0;
				document.getElementById("cupo_disponible_fijacion").value = 0;				
			}
			if (document.getElementById('producto').value == "cafe pergamino seco" || document.getElementById('producto').value == "cacao seco" ) 
			{ document.getElementById("unidad").value = "Kilos"; 
			  document.getElementById("latas_compra").disabled=true;
...
<?php
	...
	if($accion == 4)
	{
		...
		$resultados = mysqli_query($conexion," SELECT * FROM asociados WHERE cedula_usuario LIKE '%$mi_busqueda%'");
		if($consulta = mysqli_fetch_array($resultados))
		{
		   ...
		   {  
			...
			$cantidad = 0; $precio=0; $kilos = 0;  $total_fijar = 0; 
			$papa = mysqli_query($conexion,"select *from fijar WHERE cedula = $consulta[cedula_usuario]");						
			while ($papa1 = mysqli_fetch_array($papa))
			{ $cantidad= $papa1['cantidad']; $cedula = $papa1['cedula']; $nombre = $papa1['nombre']; $fecha_inicial = $papa1['fecha'];  $precio = $papa1['precio']; $producto = $papa1['producto']; $hora = $papa1['hora'];

			
			$mama = mysqli_query($conexion,"select *from liquidacion a WHERE cast(A.fecha_liquidacion as date)BETWEEN '$fecha_inicial' AND '$fecha' and cedula_liquidacion = $cedula and producto_liquidacion = '$producto' and hora_liquidacion >= '$hora' " );
			while($mama1 = mysqli_fetch_array($mama))
			{  $kilos = $kilos + $mama1['kilos_liquidacion']; }    

			   $total_fijar = $cantidad - $kilos ;

			   if ($total_fijar <= 0) 
			   { $precio=0; $kilos = 0;  $total_fijar = 0; $cantidad = 0; }
		}

		///////////////////////////////////////////////////CUPOS CAFE/////////////////////////////

		$cupo1 = mysqli_query($conexion,"select *from asociados where cedula_usuario LIKE '%$mi_busqueda%' ");
		$cupodisponiblecafe = 0; $cupodisponiblecacao = 0; $cupousadocacao = 0; $cupousadocafe = 0; $cupocafe = 0; $cupocacao = 0; 
		
		if($cupo2 = mysqli_fetch_array($cupo1))
		{   $cupocacao= $cupo2['cupo_cacao']; $cupocafe= $cupo2['cupo_cafe']; $estatus = $cupo2['estatus']; $cedula= $cupo2['cedula_usuario']; $nombre = $cupo2['nombre_usuario'];
			
			$cupo3 = mysqli_query($conexion,"select *from liquidacion where cedula_liquidacion LIKE '%$mi_busqueda%' ");
			while($cupo4 = mysqli_fetch_array($cupo3))
			{ 
			   $x=date("Y", strtotime($cupo4['fecha_liquidacion'])); 		   

		     if ($x== $anio and $estatus == "organico") 
		     {
			   if ($cupo4['producto_liquidacion'] == "cafe pergamino seco" or $cupo4['producto_liquidacion']=="cafe mojado") 
			   { $cupousadocafe= $cupousadocafe + $cupo4['kilos_liquidacion'];}

			   if ($cupo4['producto_liquidacion'] == "cacao seco" or $cupo4['producto_liquidacion'] == "cacao en baba") 
			   { $cupousadocacao= $cupousadocacao + $cupo4['kilos_liquidacion'];  }  
			 ...
			 echo 
			 '
			 <h2 style="margin-left:-40px;">REGISTRO</h2>
		...
		<div class="row g-3 ">
			 ...
			 <div class="col-lg-4">
			 <div class="input-group mb-3 form-floating">
			 <select class="form-control" name="producto" id="producto" onChange="fijar()" required>
			 <option></option>
		...
		<div class="row g-4">		  
			...
			<div class="col-lg-3">
			<div class="input-group mb-3 form-floating">
			<input type="number" class="form-control" id="numero_estopas" name="numero_estopas" required><br>
			<span class="input-group-text">✎</span>
		    ...
		    <div class="col-lg-3">
		    <div class="input-group mb-3 form-floating">
		    <input type="number" class="form-control" id="numero_sacos" name="numero_sacos" required><br>
		    <span class="input-group-text">✎</span>
		...
		<div class="row g-4">       						 
		  ...
		  <span class="input-group-text">✎</span>
          <label for="kilos_latas" class="form-label">Kilos Netos/Latas</label> 
		  </div>
		  </div>
		</div>
		...
		<div class="row g-4 ">
		  <div class="col-lg-3">
		  <div class="input-group mb-3 form-floating">
		  <input type="text" class="form-control" id="precio_fijado" name="precio_fijado"  readonly><br>
		  <span class="input-group-text">📄</span>
		  <label for="precio_fijado" class="form-label">Precio Fijado</label> 
		  </div>
		  </div>

		  <div class="col-lg-3">
		  <div class="input-group mb-3 form-floating">
		  <input type="text" class="form-control" id="cantidad_Fijada" name="cantidad_fijada"  readonly><br>
		  <span class="input-group-text">📄</span>
		  <label for="cantidad_fijada" class="form-label">Cantidad Fijada</label> 
		  </div>
		  </div>

		  <div class="col-lg-3">
		  <div class="input-group mb-3 form-floating">
		  <input type="text" class="form-control" id="kilos_vendidos" name="kilos_vendidos"  readonly><br>
		  <span class="input-group-text">📄</span>
		  ...
		  <div class="col-lg-3">
		  <div class="input-group mb-3 form-floating">
		  <input type="text" class="form-control" id="cupo_disponible_fijacion" name="cupo_disponible_fijacion"  readonly><br>
		  <span class="input-group-text">📄</span>
		...
		<div class="row g-3">
		  <div class="col-lg-4">
		  <div class="input-group mb-3 form-floating">
		  <input type="text" class="form-control" id="cupo_cafe" name="cupo_cafe" value="'.$cupocafe.' " readonly><br>
		  <span class="input-group-text">📄</span>
		  <label for="cupo_cafe" class="form-label">Cupo Cafe</label> 
		  </div>
		  </div>

		  <div class="col-lg-4">
		  <div class="input-group mb-3 form-floating">
		  <input type="text" class="form-control" id="cafe_vendido" name="cafe_vendido" value="'.$cupousadocafe.' " readonly><br>
		  <span class="input-group-text">📄</span>
		  <label for="cafe_vendido" class="form-label">Cafe vendido</label> 
		  </div>
		  </div>

		  <div class="col-lg-4">
		  <div class="input-group mb-3 form-floating">
		  <input type="text" class="form-control" id="cupo_disponible_cafe" name="cupo_disponible_cafe" value="'.$cupodisponiblecafe.' " readonly><br>
		  <span class="input-group-text">📄</span>
		  <label for="cupo_disponible" class="form-label">Cupo Disponible</label> 
		  </div>
		  </div>
	  ...
	  <div class="row g-3">
		<div class="col-lg-4">
		<div class="input-group mb-3 form-floating">
		<input type="text" class="form-control" id="cupo_cacao" name="cupo_cacao" value="'.$cupocacao.' " readonly><br>
		<span class="input-group-text">📄</span>
		<label for="cupo_cacao" class="form-label">Cupo Cacao</label> 
		</div>
		</div>

		<div class="col-lg-4">
		<div class="input-group mb-3 form-floating">
		<input type="text" class="form-control" id="cacao_vendido" name="cacao_vendido" value="'.$cupousadocacao.' " readonly><br>
		<span class="input-group-text">📄</span>
		<label for="cacao vendido" class="form-label">Cacao vendido</label> 
		</div>
		</div>

		<div class="col-lg-4">
		<div class="input-group mb-3 form-floating">
		<input type="text" class="form-control" id="cupo_disponible_cacao" name="cupo_disponible_cacao" value="'.$cupodisponiblecacao.'" readonly><br>
		<span class="input-group-text">📄</span>
		<label for="cupo_disponible" class="form-label">Cupo Disponible</label> 
		</div>
		</div>

		<input type="hidden" class="form-control" id="precio_fijado1" name="precio_fijado1" value="'.$precio.'" readonly><br>
		<input type="hidden" class="form-control" id="cantidad_Fijada1" name="cantidad_fijada1" value="'.$cantidad.'" readonly><br>
		<input type="hidden" class="form-control" id="kilos_vendidos1" name="kilos_vendidos1" value="'. $kilos.' " readonly><br>
		<input type="hidden" class="form-control" id="cupo_disponible_fijacion1" name="cupo_disponible_fijacion1" value="'.$total_fijar.'" readonly><br>
		<input type="hidden" class="form-control" id="producto1" name="producto1" value="'.$producto.'" readonly><br>

	  </div>
	  <div class="row g-1" style="margin-left:150px;">
		  <div class="col-lg-8" >		  
...
{ 		
 		$resultados = mysqli_query($conexion," SELECT * FROM compra WHERE marca LIKE '%$mi_busqueda%'");
		if($consulta = mysqli_fetch_array($resultados))
		{ 
			...
			$cupo1 = mysqli_query($conexion,"select *from liquidacion join asociados on cedula_liquidacion = cedula_usuario where cedula_usuario LIKE '%$mi_busqueda%'");
            $cupodisponiblecafe=0; $cupodisponiblecacao=0; $cupousadocacao=0; $cupousadocafe=0; $cupocacao= 0; $cupocafe= 0;
		    ...
		    {  
                ...
                $cedula = $cupo2['cedula_liquidacion']; $nombre = $cupo2['nombre_usuario'];
			...
			$destarar = mysqli_query($conexion," SELECT  *FROM analisisfisico where codigo_analisisfisico LIKE '%$mi_busqueda%' ");
			if($destarar1 = mysqli_fetch_array($destarar))
			{
				$kilos_netos = $kilos_netos - $destarar1['destare']; 	
			} 

			 $cupodisponiblecafe =  $cupocafe - $cupousadocafe - $kilos_netos;
			 $cupodisponiblecacao = $cupocacao - $cupousadocacao - $kilos_netos;

			if ($cupodisponiblecafe <0 and $estatus == "organico") 
			{ $tipo_grano = "estandar";  $prima_producto=0; }

			if ($cupodisponiblecacao <0 and $estatus=="organico") 
			{ $tipo_grano = "estandar";  $prima_producto=0; }
			...
			$fijacion="NO";
			$papa = mysqli_query($conexion,"select *from fijar WHERE cedula = $consulta[cedula]");						
			while ($papa1 = mysqli_fetch_array($papa))
			{ $cantidad= $papa1['cantidad']; $cedula = $papa1['cedula']; $nombre = $papa1['nombre']; $fecha_inicial = $papa1['fecha'];  $precio = $papa1['precio']; $hora = $papa1['hora'];
			
			  $kilos = 0;  $total_fijar = 0; 
			  $mama = mysqli_query($conexion,"select *from liquidacion a WHERE cast(A.fecha_liquidacion as date)BETWEEN '$fecha_inicial' AND '$fecha' and cedula_liquidacion = $cedula and producto_liquidacion = '$producto' and hora_liquidacion >= '$hora' ");
			  while($mama1 = mysqli_fetch_array($mama))
			  {  $kilos = $kilos + $mama1['kilos_liquidacion']; }    

			   $total_fijar = $cantidad - $kilos - $kilos_netos;
			   $total_fijar1 = $cantidad - $kilos;

			   if ($total_fijar1 >= 0  and $producto == $papa1['producto'])
			   {?>
			   <script>
				alert('Este Producto se encuentra fijado');
			   </script>
               <?php				
			   }
			   if ($total_fijar >= 0 and $producto == $papa1['producto']) 
			   { $precio_dia = $precio; $fijacion="SI";}
		   }
		    ...
		    { 
             ...
             if ($factor == 0 and $consulta['producto'] == "cafe pergamino seco") 
			     {echo "<script> analisis_fisico(); </script>";}
			 else
			 {		
			...
			echo 
			'
			<h2 style="margin-left:-200px;">LIQUIDACIÓN</h2>
			...
			<div class="form-group row g-4">
			 ...
			 <div class="col-lg-3">
			 <div class="input-group mb-3 form-floating">
		     <input type="number" class="form-control" id="kilos_liquidacion" name="kilos_liquidacion" value="'.$kilos_netos.'" required><br>
			 <span class="input-group-text">📄</span>
		  ...
		  <div class="form-group row g-4">
		     ...
		     <div class="col-lg-3">
		     <div class="input-group mb-3 form-floating">
		     <input type="text" class="form-control" id="fijacion" name="fijacion" value="'.$fijacion.'" readonly ><br>
		     <span class="input-group-text">📄</span>
		     <label for="fijacion" class="form-label">Fijacion:</label>
		     </div>
		     </div>
			 ...
			 <div class="col-lg-3">
			 <div class="input-group mb-3 form-floating">
		     <input type="number" class="form-control" id="factor" name="factor"  value="'.$factor.'" readonly><br>
			 <span class="input-group-text">📄</span>
			 ...
			 <div class="col-lg-3">
			 <div class="input-group mb-3 form-floating">
		     <input type="text" class="form-control" id="rendimiento" name="rendimiento" readonly value="'.round($rendimiento).'" readonly><br>
			 <span class="input-group-text">📄</span>
		 ...
		 <div class="form-group row g-4">	 
			...
			<div class="form-group row">
			   ...
			   <input type="hidden" id="precio_final1" name="precio_final1"><br>
			   <input type="hidden" id="subtotal1" name="subtotal1"><br>
			   <input type="hidden" id="dian1" name="dian1"><br>
               <input type="hidden" id="aporte1" name="aporte1"><br>
			   <input type="hidden" id="asociados1" name="asociados1"><br>
			   <input type="hidden" id="neto_pagar1" name="neto_pagar1"><br>
			   <input type="hidden" id="total_ahorro1" name="total_ahorro1"><br>
			   <input type="hidden" id="neto_pagar21" name="neto_pagar21"><br>
			   <input type="hidden" id="municipio_liquidacion" name = "municipio_liquidacion" value="'.$_SESSION['municipio'].'"><br>
		    </div>
...
if($accion == 12)
 {              
            ...
            {   
                if($_POST['total_ahorro1']>0)
                {mysqli_query($conexion," insert into ahorro (cedula_ahorro, nombre_ahorro, valor_ahorro, fecha_ahorro, hora_ahorro, codigo_ahorro, kilos_ahorro, cantidad_ahorro) value ($_POST[cedula_liquidacion],'$_POST[nombre_liquidacion]',$_POST[total_ahorro1],'$fecha','$hora','$_POST[codigo_liquidacion]',$_POST[kilos_liquidacion],$_POST[ahorro])");}
                
                mysqli_query($conexion,"insert into liquidacion (codigo_liquidacion,precio_dia,rendimiento,bonificacion,prima,precio_final,subtotal,fecha_liquidacion,hora_liquidacion,politica,observaciones_liquidacion,dian,asociados,neto_pagar,ahorro, total_ahorro, kilos_liquidacion, cedula_liquidacion, aporte, nombre_liquidacion, producto_liquidacion, descuento_secado, municipio_liquidacion)
                values ('$_POST[codigo_liquidacion]',$_POST[precio_dia],$_POST[rendimiento],$_POST[bonificacion],$_POST[prima_producto],$_POST[precio_final1],$_POST[subtotal1],'$fecha' ,'$hora','$_POST[politica]','$_POST[observaciones_liquidacion]',$_POST[dian1],$_POST[asociados1], $_POST[neto_pagar21],$_POST[ahorro],$_POST[total_ahorro1],$_POST[kilos_liquidacion], $_POST[cedula_liquidacion],$_POST[aporte1],'$_POST[nombre_liquidacion]','$_POST[producto_liquidacion]', $_POST[descuento_secado],'$_POST[municipio_liquidacion]')"); 
...
{ 		
		...
		$resultados = mysqli_query($conexion," SELECT * FROM compra WHERE marca LIKE '%$mi_busqueda%'");
		if($consulta = mysqli_fetch_array($resultados))
		{ $producto=$consulta['producto']; $tipo_grano=$consulta['tipo_grano']; $kilos=$consulta['kilos_netos'];
			$resul = mysqli_query($conexion," SELECT * FROM asociados WHERE cedula_usuario=$consulta[cedula]");
			if($consul= mysqli_fetch_array($resul))
			...
			echo 
			'
			<h2 style="margin-left: 400px;">ANALISIS FISICO</h2>
	   ...
	   <div class="form-group row g-4">
		 ...
		 <div class="form-group row g-4"> 
		    ...
		    <div class="col-lg-3">
		    <div class="input-group mb-3 form-floating">
		    <input type="number" class="form-control" id="kilos" name="kilos" value="'.$kilos.'" readonly><br>
		    <span class="input-group-text">📄</span>
		    <label for="kilos" class="form-label">Kilos Netos:</label>
		    </div>
		    </div>

		    <div class="col-lg-3">
		    <div class="input-group mb-3 form-floating">
		    <input type="number" class="form-control" id="destare" name="destare" onkeyup="dest();"  required><br>
		    <span class="input-group-text">✎</span>
		    <label for="destare" class="form-label">Destare:</label>
		    </div>
		    </div>

			<div class="col-lg-3">
		    <div class="input-group mb-3 form-floating">
		    <input type="text" class="form-control" id="t_kilos" name="t_kilos" readonly><br>
		    <span class="input-group-text">📄</span>
		    <label for="t_liquidar" class="form-label">Total a Liquidar:</label>
		    </div>
		    </div>
...
{ 		
		...
		{ $producto=$consulta['producto']; $tipo_grano=$consulta['tipo_grano'];
			...
			<div class="form-group row">
	             ...
	             <div class="col-lg-8">
			     <textarea class="form-control" name="notas" id="notas" rows="2" cols="60" placeholder="descriptores de calidad" required></textarea><br><br>
				 </div> 
			</div> 
  ...
  if($accion == 14)
	{
		   ...
		   {  
			...
			{   $a=$a+1;
             ...
             <div class="form-group row g-4 ">
                  <div class="col-lg-3">
                  <div class="input-group mb-3 form-floating">
                  <input type="text" class="form-control" id="tipo_usuario" name="tipo_usuario" value="'.$consulta['estatus'].'" readonly><br><br>
                  <span class="input-group-text">📄</span>
                  <label for="t_usuario" class="form-label">T. Usuario:</label> 
                  </div>
                  </div>
                  ...
                  <span class="input-group-text">✎</span>
                  <label for="n_latas" class="form-label">N. Latas:</label> 
                  </div>
                  </div>
             </div>
             ...
             <div class="form-group row g-3">
                  ...
                  <span class="input-group-text">✎</span>
                  <label for="t_humedad" class="form-label">T. Humedad:</label> 
                  </div>
                  </div>
                  ...
                  <span class="input-group-text">✎</span>
                  <label for="t_secado" class="form-label">T. Secado:</label> 
                  </div>
                  </div>

				  <div class="col-lg-6">
                  <div class="input-group mb-3 form-floating">
                  <textarea class="form-control" name="observaciones_secado" id="observaciones_secado" rows="4" cols="60" placeholder="observaciones"></textarea><br><br>
                  <span class="input-group-text">✎</span>
	 ...
	 if($accion == 15)
	 {
		...
		{ 
			...
			<div class="row g-3 ">
			     <div class="col-lg-5">
                 <div class="input-group mb-3 form-floating">
			     <input type="text" class="form-control"  id="nombre_resultado" name="nombre_resultado" value="'.$consul['nombre_usuario'].'" readonly>
		         <span class="input-group-text">📄</span>
                 <label for="nombre" class="form-label">Nombre:</label>
				 </div>
		         </div>
		         ...
		         <div class="col-lg-3">
		         <div class="input-group mb-3 form-floating">
			     <input type="number" class="form-control"  id="cedula_resultado" name="cedula_resultado" value="'.$consul['cedula_usuario'].'" readonly>
		         <span class="input-group-text">📄</span>
                 <label for="Cedula" class="form-label">Cedula:</label>
				 </div>
	             </div>			
	       ...
	       <div class="form-group row g-3">
				...
				<div class="col-lg-4">
		        <div class="input-group mb-3 form-floating">
		        <input type="text" class="form-control" id="latas" name="latas" value="'.$consulta['numero_latas'].'" readonly><br><br>
		        <span class="input-group-text">📄</span>
                <label for="numero_latas" class="form-label">Numero Latas:</label>
				</div>
	            </div>
		  ...
		  <div class="form-group row g-3">	  
		        ...
		        <span class="input-group-text">✎</span>
                <label for="v_kilo" class="form-label">Valor Kilo:</label>
				</div>
                </div>
		        ...
		        <span class="input-group-text">✎</span>
                <label for="cantidad" class="form-label">Kilos Netos:</label>
				</div>
                </div>
	        ...
	        <div class="form-group row g-2">
				 ...
				 <span class="input-group-text">✎</span>
                 <label for="tipo pago" class="form-label">Tipo Pago:</label>
				 </div>
                 </div>

				<div class="col-lg-4">
				<div class="input-group mb-3 form-floating">
				<input type="text" class="form-control" id="sacos" name="sacos"  placeholder="0" required><br><br>
				<span class="input-group-text">📄</span>
				<label for="numero_sacos" class="form-label">Numero Sacos:</label>
				</div>
				</div>
				...
				<div class="col-lg-4">
				<div class="input-group mb-3 form-floating">
				<input type="text" class="form-control" id="kilos_sobrantes" name="kilos_sobrantes"  placeholder="0" required><br><br>
				<span class="input-group-text">📄</span>
				<label for="Kilos_sobrantes" class="form-label">Kilos Sobrantes:</label>
				</div>
				</div>
			</div>
			<div class="form-group row g-1">
				 ...
				 <div class="col-lg-4">
				 <div class="input-group mb-3 form-floating">
				 <input type="hidden" class="form-control" id="municipio_resultado" name="municipio_resultado" value="'.$_SESSION['municipio'].'" readonly>
				 </div>
				 </div>
				 <input type="hidden" id="costo_total_resultado1" name="costo_total_resultado1">
			</div>
	 ...
	 if($accion == 7)
	 {
		...
		{ 
		  mysqli_query($conexion,"insert into compra (municipio_compra, cedula, producto, marca, fecha_compra, kilos_netos, tipo_grano, nombre_compra)
		  values ('$_POST[municipio_resultado]',$_POST[cedula_resultado],  '$_POST[producto]', '$_POST[codigo_resultado]', '$fecha', $_POST[cantidad_resultado], '$_POST[tipograno_resultado]', '$_POST[nombre_resultado]')"); 
		  mysqli_query($conexion,"insert into resultadosecado (codigo_resultado, fecha_resultado, cantidad_resultado, costo_resultado, costo_total_resultado, tipograno_resultado, observaciones_resultado, tipo_pago, nombre_resultado, sacos, kilos_sobrantes, producto, latas) values ('$_POST[codigo_resultado]','$fecha', $_POST[cantidad_resultado], $_POST[costo_resultado],$_POST[costo_total_resultado1],'$_POST[tipograno_resultado]','$_POST[observaciones_resultado]', '$_POST[tipo_pago]','$_POST[nombre_resultado]', $_POST[sacos], $_POST[kilos_sobrantes],'$_POST[producto]',  $_POST[latas])"); 
		  echo "<script> alert ('Registro Exitoso'); </script>";
		} 
	  ...
	  if($accion == 16)
      { 
		$resultado = mysqli_query($conexion," SELECT * FROM asociados WHERE codigo_up= '$_POST[cedula_usuario]'");
		if($consulta = mysqli_fetch_array($resultado))
		{ echo "<script> alert ('este registro ya existe'); </script>"; } 
		else 
		{	   mysqli_query($conexion,"insert into asociados (fecha_registro, codigo_up, nombre_usuario, municipio, vereda, finca, cedula_usuario, contacto, tipo_vinculacion, estatus) values ('$fecha','$_POST[cedula_usuario]', '$_POST[nombre_usuario]','$_POST[municipio]','$_POST[vereda]','$_POST[finca]',$_POST[cedula_usuario], $_POST[contacto],'$_POST[tipo_vinculacion]','estandar')");   
	   ...
	   if($accion == 17)
	   { 
		mysqli_query($conexion,"insert into producto2 (tipo, precio, descripcion, prima_producto, politica, nombre) values ('$_POST[tipo]', $_POST[precio], '$_POST[descripcion]', $_POST[prima_producto],'$_POST[politica]','$_POST[nombre]')");   
		...
		{ $total_retiro = $total_retiro + $consul['valor_retiro'];}
          ...
          $neto_ahorro=$total_ahorrado - $total_retiro;		
	        ...
	        <div class="form-group row g-3">
				 ...
				 <span class="input-group-text">✎</span>
                 <label for="retiro" class="form-label">Valor a Retirar:</label>
                 </div>   
		         </div>
		         ...
		         <span class="input-group-text">📄</span>
                 <label for="saldo" class="form-label">Saldo Disponible:</label>
				 </div>
                 </div>
			</div>
	 ...
	 {
		$mi_busqueda = $_POST['mi_busqueda'];
		$resultado = mysqli_query($conexion," SELECT  *FROM asociados where cedula_usuario LIKE '%$mi_busqueda%'");
		if($consulta = mysqli_fetch_array($resultado))
		{	
			if ($consulta['estatus']!= "organico") 
			{ $estatus="estandar"; }
			else { $estatus=$consulta['estatus']; }	
			
			echo 
			'
			<h2 class="text-center">FIJACIÓN</h2>
			<form>       

			<div class="form-group row g-3">
			     <div class="col-lg-5">
                 <div class="input-group mb-3 form-floating">
			     <input type="text" class="form-control"  id="nombre" name="nombre" value="'.$consulta['nombre_usuario'].'" readonly ><br>
		         <span class="input-group-text">📄</span>
                 <label for="nombre" class="form-label">Nombre:</label>
				 </div>
		         </div>
		         ...
		         <div class="col-lg-3">
		         <div class="input-group mb-3 form-floating">
			     <input type="number" class="form-control"  id="cedula" name="cedula" value="'.$consulta['cedula_usuario'].'" readonly ><br>
		         <span class="input-group-text">📄</span>
                 <label for="Cedula" class="form-label">Cedula:</label>
				 </div>
	             </div>     
				 ...
				 <div class="col-lg-4">
				 <div class="input-group mb-3 form-floating">
				 <input type="text" class="form-control" id="municipio" name="municipio" value="'.$_SESSION['municipio'].'" readonly><br>
				 <span class="input-group-text">📄</span>
				 <label for="Municipio" class="form-label">Municipio:</label>
				 </div>
				 </div>
	       ...
	       <div class="form-group row g-3">
				...
				<div class="col-lg-3">
		        <div class="input-group mb-3 form-floating">
		        <input type="text" class="form-control" id="estatus" name="estatus" value="'.$estatus.'" readonly><br><br>
		        <span class="input-group-text">📄</span>
                <label for="Estatus" class="form-label">Estatus:</label>
				</div>
	            </div>
				...
				<div class="col-lg-3">
				<div class="input-group mb-3 form-floating">
				<input type="text" class="form-control" id="cantidad" name="cantidad" value="'.$consulta['cupo_cafe'].'" required><br><br>
				<span class="input-group-text">✎</span>
				<label for="Kilos_fijar" class="form-label">Kilos a Fijar:</label>
				</div>
				</div>
			...
			<div class="form-group row">
			     <div class="offset-lg-5 col-lg-8">
			  	 <button class="btn btn-secondary btn-lg" onclick="fijacion();">Guardar</button>
			     </div> 
			     </div>
			     </form>
			  	';
...
if($accion == 25)
	...
	{
		$resultado = mysqli_query($conexion," SELECT  *FROM producto where nombre ='$_POST[producto]' and tipo = '$_POST[estatus]'");
			 
		if ($consulta = mysqli_fetch_array($resultado))
		  {
			mysqli_query($conexion,"insert into fijar (nombre, cedula, cantidad, fecha, anio, municipio, producto, estatus, precio, hora) 
			values ('$_POST[nombre]',$_POST[cedula],$_POST[cantidad],'$fecha','$anio','$_POST[municipio]','$_POST[producto]','$_POST[estatus]',$consulta[precio],'$hora')");  
			echo "<script> alert('Registro exitoso');</script>";
		  }  
	}   
